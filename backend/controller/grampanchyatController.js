...
exports.addUser = async (req, res) => {
  try {
    ...
    const { consumerName, address, aadhar, contact, email } = req.body;
...
exports.createAnnouncement = async (req, res) => {
  try {
    ...
    // Get the list of users associated with the GP
    const gp = await Grampanchayat.findById(gpId).populate('userList');
    const users = gp.userList;

    // Update each user with the new announcement ID
    await Promise.all(
      users.map((user) =>
        User.findByIdAndUpdate(user._id, {
          $push: { notification: savedAnnouncement._id },
        })
      )
    );
...
exports.createFundRequest = async (req, res) => {
  try {
    const gpId = req.user.id; // Authenticated GP ID
    const { category, amount, description, phedId } = req.body;

    // Input validation
    if (!category || !amount || !description || !phedId) {
      return res.status(400).json({
        message: "All fields are required: category, amount, description, and phedId.",
      });
    }
...
exports.getGpNotifications = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: gp.notification, // Return populated notifications
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({
...
exports.createGpComplaint = async (req, res) => {
  try {
    ...
    const newComplaint = new GpComplaint({
      category,
      message,
      status: "Pending", // Default status is false (pending)
      gpName: gp.name,
    });
...
// List GP Complaints
exports.complaintList = async (req, res) => {
  try {
    const gpId = req.user.id; // Assuming the GP's ID comes from the authenticated user

    // Find the GP and populate the 'complaint' field
    const gp = await Grampanchayat.findById(gpId).populate('complaint');
